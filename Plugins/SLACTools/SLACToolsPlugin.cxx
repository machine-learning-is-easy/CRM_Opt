...
namespace
{
  ...
  class SLACToolsPluginCleaner
  {
  public:
    SLACToolsPluginCleaner() = default;
    ~SLACToolsPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("SLACTools");
    }
  } SLACToolsPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "SLACTools_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "SLACTools_server_manager_modules.h"
#include "SLACTools_client_server.h"
#endif

//-----------------------------------------------------------------------------
void SLACToolsPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = SLACTools_qch();
...
#define PARAVIEW_ADD_INTERFACES(arg) \
  (arg).push_back(new pqSLACActionGroupImplementation(this)); \
...
SLACToolsPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return SLACTools_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void SLACToolsPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  SLACTools_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  SLACTools_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList SLACToolsPlugin::interfaces()
{
  QObjectList ifaces;
...
void SLACToolsPlugin::Initialize()
{
...
void SLACToolsPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_SLACTools_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(SLACToolsPlugin)
#endif
}

//-----------------------------------------------------------------------------
SLACToolsPlugin::SLACToolsPlugin()
{
  initialize_SLACTools_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* SLACToolsPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return SLACTools_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(SLACTools, SLACToolsPlugin)
