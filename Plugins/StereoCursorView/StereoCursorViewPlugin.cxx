#include "StereoCursorViewPlugin.h"
...
namespace
{
  ...
  class StereoCursorViewPluginCleaner
  {
  public:
    StereoCursorViewPluginCleaner() = default;
    ~StereoCursorViewPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("StereoCursorView");
    }
  } StereoCursorViewPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "StereoCursorView_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "StereoCursorView_server_manager_modules.h"
#include "StereoCursorView_client_server.h"
#endif

//-----------------------------------------------------------------------------
void StereoCursorViewPlugin::GetBinaryResources(
...
StereoCursorViewPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return StereoCursorView_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void StereoCursorViewPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  StereoCursorView_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  StereoCursorView_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList StereoCursorViewPlugin::interfaces()
{
  QObjectList ifaces;
...
void StereoCursorViewPlugin::Initialize()
{
...
void StereoCursorViewPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_StereoCursorView_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(StereoCursorViewPlugin)
#endif
}

//-----------------------------------------------------------------------------
StereoCursorViewPlugin::StereoCursorViewPlugin()
{
  initialize_StereoCursorView_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* StereoCursorViewPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return StereoCursorView_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(StereoCursorView, StereoCursorViewPlugin)
