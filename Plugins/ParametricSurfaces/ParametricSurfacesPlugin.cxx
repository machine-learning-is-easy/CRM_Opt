#include "ParametricSurfacesPlugin.h"
...
#include "ParametricSurfaces_qch.h"
...
namespace
{
  ...
  class ParametricSurfacesPluginCleaner
  {
  public:
    ParametricSurfacesPluginCleaner() = default;
    ~ParametricSurfacesPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("ParametricSurfaces");
    }
  } ParametricSurfacesPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "ParametricSurfaces_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "ParametricSurfaces_server_manager_modules.h"
#include "ParametricSurfaces_client_server.h"
#endif

//-----------------------------------------------------------------------------
void ParametricSurfacesPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = ParametricSurfaces_qch();
...
ParametricSurfacesPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return ParametricSurfaces_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void ParametricSurfacesPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  ParametricSurfaces_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  ParametricSurfaces_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList ParametricSurfacesPlugin::interfaces()
{
  QObjectList ifaces;
...
void ParametricSurfacesPlugin::Initialize()
{
...
void ParametricSurfacesPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_ParametricSurfaces_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(ParametricSurfacesPlugin)
#endif
}

//-----------------------------------------------------------------------------
ParametricSurfacesPlugin::ParametricSurfacesPlugin()
{
  initialize_ParametricSurfaces_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* ParametricSurfacesPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return ParametricSurfaces_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(ParametricSurfaces, ParametricSurfacesPlugin)
