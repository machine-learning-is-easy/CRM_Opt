...
namespace
{
  ...
  class MomentsPluginCleaner
  {
  public:
    MomentsPluginCleaner() = default;
    ~MomentsPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("Moments");
    }
  } MomentsPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "Moments_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "Moments_server_manager_modules.h"
#include "Moments_client_server.h"
#endif

//-----------------------------------------------------------------------------
void MomentsPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = Moments_qch();
...
MomentsPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return Moments_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void MomentsPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  Moments_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  Moments_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList MomentsPlugin::interfaces()
{
  QObjectList ifaces;
...
void MomentsPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_Moments_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
...
{
  initialize_Moments_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* MomentsPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return Moments_EULA;
#else
...
PV_PLUGIN_EXPORT(Moments, MomentsPlugin)
