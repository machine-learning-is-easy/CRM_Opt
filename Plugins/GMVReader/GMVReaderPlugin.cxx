...
namespace
{
  ...
  class GMVReaderPluginCleaner
  {
  public:
    GMVReaderPluginCleaner() = default;
    ~GMVReaderPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("GMVReader");
    }
  } GMVReaderPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "GMVReader_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "GMVReader_server_manager_modules.h"
#include "GMVReader_client_server.h"
#endif

//-----------------------------------------------------------------------------
void GMVReaderPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = GMVReader_qch();
...
GMVReaderPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return GMVReader_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void GMVReaderPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  GMVReader_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  GMVReader_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList GMVReaderPlugin::interfaces()
{
  QObjectList ifaces;
...
void GMVReaderPlugin::Initialize()
{
...
void GMVReaderPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_GMVReader_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(GMVReaderPlugin)
#endif
}

//-----------------------------------------------------------------------------
GMVReaderPlugin::GMVReaderPlugin()
{
  initialize_GMVReader_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* GMVReaderPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return GMVReader_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(GMVReader, GMVReaderPlugin)
