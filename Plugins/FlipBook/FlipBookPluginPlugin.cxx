#include "FlipBookPluginPlugin.h"
...
namespace
{
  ...
  class FlipBookPluginPluginCleaner
  {
  public:
    FlipBookPluginPluginCleaner() = default;
    ~FlipBookPluginPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("FlipBookPlugin");
    }
  } FlipBookPluginPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "FlipBookPlugin_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "FlipBookPlugin_server_manager_modules.h"
#include "FlipBookPlugin_client_server.h"
#endif

//-----------------------------------------------------------------------------
void FlipBookPluginPlugin::GetBinaryResources(
...
#define PARAVIEW_ADD_INTERFACES(arg) \
  (arg).push_back(new pqFlipBookToolbarActionsImplementation(this)); \
...
FlipBookPluginPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return FlipBookPlugin_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void FlipBookPluginPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  FlipBookPlugin_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  FlipBookPlugin_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList FlipBookPluginPlugin::interfaces()
{
  QObjectList ifaces;
...
void FlipBookPluginPlugin::Initialize()
{
...
void FlipBookPluginPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_FlipBookPlugin_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(FlipBookPluginPlugin)
#endif
}

//-----------------------------------------------------------------------------
FlipBookPluginPlugin::FlipBookPluginPlugin()
{
  initialize_FlipBookPlugin_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* FlipBookPluginPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return FlipBookPlugin_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(FlipBookPlugin, FlipBookPluginPlugin)
