...
#include "ThickenLayeredCells_qch.h"
...
namespace
{
  ...
  class ThickenLayeredCellsPluginCleaner
  {
  public:
    ThickenLayeredCellsPluginCleaner() = default;
    ~ThickenLayeredCellsPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("ThickenLayeredCells");
    }
  } ThickenLayeredCellsPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "ThickenLayeredCells_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "ThickenLayeredCells_server_manager_modules.h"
#include "ThickenLayeredCells_client_server.h"
#endif

//-----------------------------------------------------------------------------
void ThickenLayeredCellsPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = ThickenLayeredCells_qch();
...
ThickenLayeredCellsPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return ThickenLayeredCells_client_server_initialize;
#else
  return nullptr;
#endif
}
...
void ThickenLayeredCellsPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  ThickenLayeredCells_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  ThickenLayeredCells_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList ThickenLayeredCellsPlugin::interfaces()
{
  QObjectList ifaces;
...
void ThickenLayeredCellsPlugin::Initialize()
{
...
void ThickenLayeredCellsPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_ThickenLayeredCells_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(ThickenLayeredCellsPlugin)
#endif
}
...
{
  initialize_ThickenLayeredCells_plugin_qt_resources();
}
...
{
#if _paraview_add_plugin_EULA
  return ThickenLayeredCells_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(ThickenLayeredCells, ThickenLayeredCellsPlugin)
