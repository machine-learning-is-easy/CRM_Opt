#include "HyperTreeGridADRPlugin.h"
...
#include "HyperTreeGridADR_qch.h"
...
namespace
{
  ...
  class HyperTreeGridADRPluginCleaner
  {
  public:
    HyperTreeGridADRPluginCleaner() = default;
    ~HyperTreeGridADRPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("HyperTreeGridADR");
    }
  } HyperTreeGridADRPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "HyperTreeGridADR_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "HyperTreeGridADR_server_manager_modules.h"
#include "HyperTreeGridADR_client_server.h"
#endif

//-----------------------------------------------------------------------------
void HyperTreeGridADRPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = HyperTreeGridADR_qch();
...
HyperTreeGridADRPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return HyperTreeGridADR_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void HyperTreeGridADRPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  HyperTreeGridADR_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  HyperTreeGridADR_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList HyperTreeGridADRPlugin::interfaces()
{
  QObjectList ifaces;
...
void HyperTreeGridADRPlugin::Initialize()
{
...
void HyperTreeGridADRPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_HyperTreeGridADR_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(HyperTreeGridADRPlugin)
#endif
}

//-----------------------------------------------------------------------------
HyperTreeGridADRPlugin::HyperTreeGridADRPlugin()
{
  initialize_HyperTreeGridADR_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* HyperTreeGridADRPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return HyperTreeGridADR_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(HyperTreeGridADR, HyperTreeGridADRPlugin)
