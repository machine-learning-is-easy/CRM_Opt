#include "GeodesicMeasurementPlugin.h"
...
#include "GeodesicMeasurement_qch.h"
...
namespace
{
  ...
  class GeodesicMeasurementPluginCleaner
  {
  public:
    GeodesicMeasurementPluginCleaner() = default;
    ~GeodesicMeasurementPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("GeodesicMeasurement");
    }
  } GeodesicMeasurementPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "GeodesicMeasurement_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "GeodesicMeasurement_server_manager_modules.h"
#include "GeodesicMeasurement_client_server.h"
#endif

//-----------------------------------------------------------------------------
void GeodesicMeasurementPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = GeodesicMeasurement_qch();
...
GeodesicMeasurementPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return GeodesicMeasurement_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void GeodesicMeasurementPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  GeodesicMeasurement_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  GeodesicMeasurement_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList GeodesicMeasurementPlugin::interfaces()
{
  QObjectList ifaces;
...
void GeodesicMeasurementPlugin::Initialize()
{
...
void GeodesicMeasurementPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_GeodesicMeasurement_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(GeodesicMeasurementPlugin)
#endif
}

//-----------------------------------------------------------------------------
GeodesicMeasurementPlugin::GeodesicMeasurementPlugin()
{
  initialize_GeodesicMeasurement_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* GeodesicMeasurementPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return GeodesicMeasurement_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(GeodesicMeasurement, GeodesicMeasurementPlugin)
