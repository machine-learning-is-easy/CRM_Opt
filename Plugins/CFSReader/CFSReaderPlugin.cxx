...
namespace
{
  ...
  class CFSReaderPluginCleaner
  {
  public:
    CFSReaderPluginCleaner() = default;
    ~CFSReaderPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("CFSReader");
    }
  } CFSReaderPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "CFSReader_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "CFSReader_server_manager_modules.h"
#include "CFSReader_client_server.h"
#endif

//-----------------------------------------------------------------------------
void CFSReaderPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = CFSReader_qch();
...
CFSReaderPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return CFSReader_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void CFSReaderPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  CFSReader_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  CFSReader_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList CFSReaderPlugin::interfaces()
{
  QObjectList ifaces;
...
void CFSReaderPlugin::Initialize()
{
...
void CFSReaderPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_CFSReader_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(CFSReaderPlugin)
#endif
}

//-----------------------------------------------------------------------------
CFSReaderPlugin::CFSReaderPlugin()
{
  initialize_CFSReader_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* CFSReaderPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return CFSReader_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(CFSReader, CFSReaderPlugin)
