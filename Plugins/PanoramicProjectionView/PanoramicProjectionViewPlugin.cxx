#include "PanoramicProjectionViewPlugin.h"
...
namespace
{
  ...
  class PanoramicProjectionViewPluginCleaner
  {
  public:
    PanoramicProjectionViewPluginCleaner() = default;
    ~PanoramicProjectionViewPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("PanoramicProjectionView");
    }
  } PanoramicProjectionViewPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "PanoramicProjectionView_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "PanoramicProjectionView_server_manager_modules.h"
#include "PanoramicProjectionView_client_server.h"
#endif

//-----------------------------------------------------------------------------
void PanoramicProjectionViewPlugin::GetBinaryResources(
...
PanoramicProjectionViewPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return PanoramicProjectionView_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void PanoramicProjectionViewPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  PanoramicProjectionView_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  PanoramicProjectionView_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList PanoramicProjectionViewPlugin::interfaces()
{
  QObjectList ifaces;
...
void PanoramicProjectionViewPlugin::Initialize()
{
...
void PanoramicProjectionViewPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_PanoramicProjectionView_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(PanoramicProjectionViewPlugin)
#endif
}

//-----------------------------------------------------------------------------
PanoramicProjectionViewPlugin::PanoramicProjectionViewPlugin()
{
  initialize_PanoramicProjectionView_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* PanoramicProjectionViewPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return PanoramicProjectionView_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(PanoramicProjectionView, PanoramicProjectionViewPlugin)
