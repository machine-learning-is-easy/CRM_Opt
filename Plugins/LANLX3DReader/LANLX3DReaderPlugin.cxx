...
namespace
{
  ...
  class LANLX3DReaderPluginCleaner
  {
  public:
    LANLX3DReaderPluginCleaner() = default;
    ~LANLX3DReaderPluginCleaner()
    {
      // The plugin library is being unloaded.
      // Let the plugin loader know so it doesn't try to unload it again.
      vtkPVPluginLoader::PluginLibraryUnloaded("LANLX3DReader");
    }
  } LANLX3DReaderPluginCleaner_Instance;
}

//-----------------------------------------------------------------------------
#if _paraview_add_plugin_SERVER_MANAGER_XML
#include "LANLX3DReader_server_manager.h"
#endif

#if _paraview_add_plugin_MODULES
#include "LANLX3DReader_server_manager_modules.h"
#include "LANLX3DReader_client_server.h"
#endif

//-----------------------------------------------------------------------------
void LANLX3DReaderPlugin::GetBinaryResources(
  std::vector<std::string>& resources)
{
  {
      const char *text = LANLX3DReader_qch();
...
LANLX3DReaderPlugin::GetInitializeInterpreterCallback()
{
#if _paraview_add_plugin_MODULES
  return LANLX3DReader_client_server_initialize;
#else
  return nullptr;
#endif
}
...
//-----------------------------------------------------------------------------
void LANLX3DReaderPlugin::GetXMLs(std::vector<std::string> &xmls)
{
  (void)xmls;
#if _paraview_add_plugin_SERVER_MANAGER_XML
  LANLX3DReader_server_manager_initialize(xmls);
#endif
#if _paraview_add_plugin_MODULES
  LANLX3DReader_server_manager_modules_initialize(xmls);
#endif
}
#endif
...
//-----------------------------------------------------------------------------
#if _paraview_add_plugin_with_ui
QObjectList LANLX3DReaderPlugin::interfaces()
{
  QObjectList ifaces;
...
void LANLX3DReaderPlugin::Initialize()
{
...
void LANLX3DReaderPlugin::GetPythonSourceList(std::vector<std::string>& modules,
...
//-----------------------------------------------------------------------------
static void initialize_LANLX3DReader_plugin_qt_resources()
{
#if !_paraview_add_plugin_built_shared && _paraview_add_plugin_with_resources
  ...
  Q_IMPORT_PLUGIN(LANLX3DReaderPlugin)
#endif
}

//-----------------------------------------------------------------------------
LANLX3DReaderPlugin::LANLX3DReaderPlugin()
{
  initialize_LANLX3DReader_plugin_qt_resources();
}

//-----------------------------------------------------------------------------
const char* LANLX3DReaderPlugin::GetEULA()
{
#if _paraview_add_plugin_EULA
  return LANLX3DReader_EULA;
#else
  return nullptr;
#endif
}
...
PV_PLUGIN_EXPORT(LANLX3DReader, LANLX3DReaderPlugin)
